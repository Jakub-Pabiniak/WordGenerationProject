...
public class WorldGeneration : MonoBehaviour
{
    ...
    public void generate_rivers(ref int[,] map)
    {
        ...
        // place river tiles and mark where they are on map[,]
        ...
        do
        {
            ...
            else // if randomvalue >= horiznotal distance indicator move Verticaly
            {
                if (isInMap(indicator[0], indicator[1])) //for security check if indicator still in map borders (by deafult it is supposed to always be)
                {
                    selectedTile = waterTiles[Random.Range(0, waterTiles.Length)]; // Randomly select a tile
                ...
                for (int i = 1; i <= (riverWidth - 1) / 2; i++)
                {
                    ...
                    {
                        selectedTile = waterTiles[Random.Range(0, waterTiles.Length)]; // Randomly select a tile
                        ground.SetTile(new Vector3Int(indicator[0] + i, indicator[1] , 0), selectedTile); // Place tile to the top of the indicator
                    ...
                    {
                        selectedTile = waterTiles[Random.Range(0, waterTiles.Length)]; // Randomly select a tile
                        ground.SetTile(new Vector3Int(indicator[0] - i, indicator[1], 0), selectedTile); // Place tile to the bottom of the indicator
                        map[indicator[0] - i, indicator[1] ] = 1; // mark where river tile is
                    }
                }
                if (chance == 0){
